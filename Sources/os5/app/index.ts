...
function setHoursMinutes(folder: font.folder) {
  ...
  // Dates
  //setDates(folder); when font will be updated
}

function setDates(folder: font.folder) {
  // Format request all the data of the last date
  const lastDate = simpleMinutes.last;
  ...
  font.print(date1, _dates1,folder);
  // Position
  _dates2Container.x = (device.screen.width / 2) - (date2.length * 10);
  font.print(date2, _dates2,folder);
}
...
simpleSettings.initialize(
  ...
  (settingsNew: Settings) => {
    ...
    if (settingsNew.showBatteryBar !== undefined) {
      _batteryBarContainer.style.display = settingsNew.showBatteryBar === true
        ? "inline"
        : "none";
    }
...
function onLeavedAOD() {
  ...
  if (_settings.showBatteryBar) _batteryBarContainer.style.display = "inline";
